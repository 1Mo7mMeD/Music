...
client2.on('message', message => {
    ...
    if (message.content.startsWith(prefix2 + 'setStreaming')) {
    ...
    } else if(message.content.startsWith(prefix2 + 'setWatching')) {
        client2.user.setActivity(argresult,{type: 'WATCHING'});

      } else if(message.content.startsWith(prefix2 + 'setListening')) {
        client2.user.setActivity(argresult,{type: 'LISTENING'});

      } else if(message.content.startsWith(prefix2 + 'setPlaying')) {
        client2.user.setActivity(argresult,{type: 'PLAYING'});

      } else if(message.content.startsWith(prefix2 + 'setName')) {
      ...
      } else if(message.content.startsWith(prefix2 + 'setStatus')) {
        if(!argresult) return message.channel.send('`online`, `DND(Do not Distrub),` `idle`, `invisible(Offline)` :notes: أختر أحد الحالات');
...
client2.on('message', async msg =>{
    ...
    .setColor(3447003)
    .setTitle("Pong!!")
    .setDescription(`${client2.ping} ms,`)
    .setFooter(`Requested by | ${msg.author.tag}`);
    msg.delete().catch(O_o=>{})
    msg.channel.send(embed);
    }
